    ...
    def sample(self, x, steps=20, t_start=None, t_end=None, order=2, skip_type='time_uniform',
        ...
        with torch.no_grad():
            ...
            if denoise_to_zero:
                ...
                if return_intermediate:
                    intermediates.append(x)
        profile = cache_dic.get('profile', None)
        if profile is not None:
            profile['flops'] = float(cache_dic.get('flops', 0.0))
            profile['tflops'] = float(cache_dic.get('flops', 0.0)) * 1e-12
            profile['model_flops'] = float(cache_dic.get('model_flops', cache_dic.get('flops', 0.0)))
            profile['model_tflops'] = float(cache_dic.get('model_flops', cache_dic.get('flops', 0.0))) * 1e-12
            profile['tcc_flops'] = float(cache_dic.get('tcc_flops', 0.0))
            profile['tcc_tflops'] = float(cache_dic.get('tcc_flops', 0.0)) * 1e-12
            profile['use_toca'] = bool(cache_dic.get('use_toca', True))
            profile['steps'] = int(steps)
