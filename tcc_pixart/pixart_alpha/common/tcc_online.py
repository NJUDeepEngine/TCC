...
class TokenPoolTccCorrector:
    def __init__(
        ...
        *,
        ...
        self.preloaded_packs = {}
        if preload:
            self.preload()

    def _load_pack(self, step: int) -> Optional[dict]:
        path = os.path.join(self.tcc_dir, f"step_{int(step):02d}.pt")
    ...
    def preload(self) -> None:
        for step in sorted(self.target_steps):
            pack = self._load_pack(step)
            if pack is not None:
                self.preloaded_packs[int(step)] = pack
    ...
    def set_step(self, step: int) -> None:
        ...
        self.current_pack = None
        if step in self.preloaded_packs:
            self.current_pack = self.preloaded_packs[step]
            return
        self.current_pack = self._load_pack(step)
